var express = require('express'),
	 OAuth = require('./node-oauth/lib/oauth.js').OAuth,
...
app.get('/google_login', function(req, res) {
	...
	oa.getOAuthRequestToken(function(error, oauth_token, oauth_token_secret, results){
	  if(error) {
	   	...
	   	res.redirect("https://www.google.com/latitude/apps/OAuthAuthorizeToken?oauth_token="+oauth_token);
	  }
	})
	...
	oa.getOAuthAccessToken(
		...
		function(error, oauth_access_token, oauth_access_token_secret, results2) {
			...
			if(error) {
	    		...
	    		res.redirect((req.param('action') && req.param('action') != "") ? req.param('action') : "/google_lat");
	 		}
...
app.get('/google_lat', require_google_login, function(req, res) {
		var oa = new OAuth(req.session.oa._requestUrl,
	                  req.session.oa._accessUrl,
	                  req.session.oa._consumerKey,
	                  req.session.oa._consumerSecret,
	...
	oa.getProtectedResource(
		"https://www.googleapis.com/latitude/v1/%s", 
		"GET", 
		req.session.oauth_access_token, 
		req.session.oauth_access_token_secret,
		function (error, data, response) {
